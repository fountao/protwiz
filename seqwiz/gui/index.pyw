...
def loadcuilis(cuilis,curdir):
    ...
    for ec in cuilis:
        ...
        try:
            tmplis=[gcidic['appinfo']['name'],gcidic['appinfo']['version'],gcidic['gcipars']['cuiapp'],cuifi,appdir,gcidic['appinfo']['description']]
        except Exception as em:
            msgdlg('CUI config file error: %s!\n%s'%(cuifi,str(em)),'Error',fatal=1)
        rtab.append(tmplis)
    return rtab
...
class MainFrame(wx.Frame): 
    def __init__(
        ...
        self.lisctrl.InsertColumn(1, 'Name', wx.LIST_FORMAT_CENTER) 
        self.lisctrl.InsertColumn(2, 'Version', wx.LIST_FORMAT_CENTER) 
        self.lisctrl.InsertColumn(3, 'CUI APP', wx.LIST_FORMAT_CENTER) 
        self.lisctrl.InsertColumn(4, 'Directory',width=wx.LIST_AUTOSIZE_USEHEADER)
    ...
    def updlist(self):
        ...
        for er in self.listab:
            ...
            self.lisctrl.SetItem(index, 1, er[0]) 
            self.lisctrl.SetItem(index, 2, er[1]) 
            self.lisctrl.SetItem(index, 3, er[2])
            #to get relative path
            rltpath=os.path.dirname(er[3].replace(os.path.dirname(self.scpdir), '.', 1))
            self.lisctrl.SetItem(index, 4, rltpath)
        ...
        self.lisctrl.SetColumnWidth(3, wx.LIST_AUTOSIZE)
    ...
    def OnSearch(self,evt):
        ...
        #search name, version and description
        self.listab=[]
        for es in self.slistab:
            if re.findall(schkey.lower(),es[-1].lower()) or re.findall(schkey.lower(),es[0].lower()) or re.findall(schkey.lower(),es[1].lower()):
