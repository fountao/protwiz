...
def sepfinder(rid,rseq,initlis,stoplis,cdsdic,initcls,minlen=6,maxlen=200,rlabel='L',slabel='F'):
                    ...
                    if (not ambi and stdcds=='S') or ambi:
                        ...
                        if tmporf[:3] in CODONDIC['Default']['Initiation']['Canonical']:
                            ...
                            if tmporf[:3] in CODONDIC['Default']['Initiation']['Near-Cognate']:
                                ...
                                initcds='A'
                        tmpid='%s:%s%s%s%s@%s-%s|%s'%(rid,slabel,ponu+1,initcds,stdcds,bnu,enu,rlabel)
...
labdic={'frame':{'F':'Forward','R':'Reverse'},
        ...
        'chain':{'L':'Linear','C':'Circular','R':'Rotative'}}
...
if __name__=='__main__':
    ...
    aparser.add_argument('-i','--init',default='N',choices=['C','N','A'],help='Level of initial codon usage: [C]anonical, [N]ear-cognate (with canonical) and [A]ll possible initial condons')
                                    ...
                                    if (not ambi and stdcds=='S') or ambi:
                                        ...
                                        if tmporf[:3] in CODONDIC['Default']['Initiation']['Canonical']:
                                            ...
                                            if tmporf[:3] in CODONDIC['Default']['Initiation']['Near-Cognate']:
                                                ...
                                                initcds='A'
                                        tmpid='%s:%s%s%s%s@%s-%s|%s'%(rid,slabel,ponu+1,initcds,stdcds,bnu,enu,rlabel)
                                        tmptlis=[tmpid,rid,labdic['frame'][slabel],ponu+1,tmporf[:3],labdic['initial'][initcds],labdic['codon'][stdcds],bnu,enu,labdic['chain'][rlabel],rlen,len(tmporf),len(tmppep)]
